...
This denormalised, wide-table approach is a pragmatic pattern that originated in the Hadoop era, where joins across distributed datasets were expensive and storage was cheap. Rather than normalising quality metadata into separate lookup tables (requiring a join at query time), you widen the row to include everything a consumer might need. The cost in storage is trivial; the benefit in query simplicity and pipeline reliability is substantial. We will return to the specific implementation of this pattern — the *flat enhanced format* — in Chapter 9.
