...
The enhanced output is a nested record format — each field in the original data becomes a JSON object containing the raw value, its masks, and any inferred rules. The *flat enhanced format* takes this a step further, serialising the nested structure into flattened key-value tuples (e.g. `fieldname.raw`, `fieldname.HU`, `fieldname.Rules.is_numeric`) so that every record is a self-contained set of pairs. This per-record, tuple-based approach means quality metadata travels with the data it describes — no joins, no lookups, no separate tables. We will return to the specific implementation of this pattern in Chapter 9.
