...
For organisations working with billions or trillions of rows, neither a browser tool nor a single-machine CLI is sufficient. The DQOR principles — mask-based profiling, the flat enhanced format, assertion rules — are straightforward to implement on distributed processing frameworks such as Apache Spark. The mask function is a stateless per-value transformation, which makes it embarrassingly parallel. The assertion rules are independent per-field. The flat enhanced output is a wide table that maps naturally to columnar storage formats like Parquet or Delta Lake. Any team with Spark expertise can implement a DQOR pipeline using the same logic described in this book, scaled to whatever volume their platform requires.

The relationship between the two tools is a product ladder:
...
Both tools implement the same profiling philosophy, the same mask functions, and the same flat enhanced output format. A profile generated by DataRadar in a browser is structurally identical to one generated by bytefreq on the command line. The techniques scale; the tools scale with them.
