...
**The format works at any scale**: as a JSON or NDJSON file from the browser tool (DataRadar), as a CSV or Parquet output from the CLI tool (bytefreq), or as a distributed dataset in Spark (DQPRE). The principle is the same at every scale; only the storage medium and the processing engine change.
